  ...
  return function () {
    ...
    this.blockMacro(function() {
      ...
      this.process((parent, target, attrs) => {
        const configurationProperties = findConfigurationProperties(parent, getScopedConfigurationProperties(context), target);
        if (configurationProperties) {
          const source = []
          source.push("[cols=\"4,3,3\", options=\"header\"]");
          source.push("|===");
          source.push("| Name | Description| Default Value");
          for (const property of configurationProperties) {
            source.push(`| \`+${property.name}+\``);
            source.push(`| ${property.description}`);
            source.push(`| ${property.defaultValue || ""}`);
          }
          source.push("|===");
          return this.parseContent(parent, source, null)
        }
      })
    })
  }
...
function getScopedConfigurationProperties ({ contentCatalog, file, config }) {
  ...
  return (cache[scope] = contentCatalog
    ...
    .reduce((accum, { contents, src }) => {
      ...
      JSON.parse(contents).properties.forEach(({ name, description, defaultValue, type = 'java.lang.Object', deprecation }) => {
        accum[name] = {
...
function findConfigurationProperties (node, configurationProperties, prefix) {
  const matches = []
  for (const [name, property] of Object.entries(configurationProperties)) {
    if (name.startsWith(prefix + '.')) {
      matches.push({ name: name, description: property.description, defaultValue: property.defaultValue});
    }
  }
  if (matches.length == 0) {
    log(node, 'warn', `found no configuration properties with prefix ${prefix}`)
  } else {
    return matches
  }
}
